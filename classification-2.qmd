...
GEE makes pre-classified products easy to access through the Earth Engine Data Catalog. For land use and land cover change analysis, [Dynamic World V1](https://developers.google.com/earth-engine/datasets/catalog/GOOGLE_DYNAMICWORLD_V1) and [ESA WorldCover 10 m v200](https://developers.google.com/earth-engine/datasets/catalog/ESA_WorldCover_v200#bands) can be used as general land classifiers. The [Global Map of Local Climate Zones](https://developers.google.com/earth-engine/datasets/catalog/RUB_RUBCLIM_LCZ_global_lcz_map_latest#bands) (LCZ) adds a different perspective by classifying areas according to urban form and surface structure, which makes it more useful for analysing neighbourhood types, urban morphology and urban climate patterns than for simple land-cover mapping. Beyond these, GEE also includes built-environment datasets such as [Open Buildings V3 Polygons](https://developers.google.com/earth-engine/datasets/catalog/GOOGLE_Research_open-buildings_v3_polygons) for building-level analysis and [TIGER: US Census Roads](https://developers.google.com/earth-engine/datasets/catalog/TIGER_2016_Roads) for road data but is limited to the US.

## Reflection

This week made me realise that classification is not only about improving accuracy, but about choosing a representation of the city that is appropriate to the question being asked. In principle, EO-derived classification could contribute much more directly to policy, especially in areas such as informal settlement monitoring, green infrastructure planning and urban expansion management, but in practice this integration is constrained by data resolution, access to reliable training data, transferability between cities, and the fact that classification outputs can still be difficult for non-specialists to interpret. I found OBIA particularly interesting because it feels closer to how planners actually read urban space, as clusters, neighbourhoods and built forms rather than isolated pixels, while sub-pixel analysis was useful in a different way because it showed that medium-resolution imagery can still produce meaningful urban information if we accept that cities are mixtures rather than clean categories. For my own work, the most useful takeaway is that there is no universally “best” classifier: the method has to match the spatial complexity of the city, the resolution of the imagery, and the kind of evidence needed for decision-making. That is probably the most important insight from this week, because it shifts classification from being a purely technical step to being a methodological choice with real consequences for how urban problems are understood.
